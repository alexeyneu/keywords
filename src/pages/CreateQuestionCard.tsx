...
import {ErrorMessageClient} from "../components/MessageClient/MessageClient";
...
const CreateQuestionCard:React.FC<props> = () => {
    ...
    const createQuestions = async (values:valueSubmit) => {
        ...
        if(isMint) {
           await save(
              ...
              {
                onSuccess:(result:any) => {
                    setStatus('You have created a word!')
                },
                 onError:(error:any) => {
                    console.log(error)
                    dispatchNotification({
    ...
    return(
        <>
            ...
            {status === 'You have created a word!' &&
                                    ...
                                    <StepsCreateWord>
                                        <div style={{maxWidth: "57rem"}}>
                                            ...
                                            <Input
                                                ...
                                                name='Word'
                                            />
                                            {errors.Word && touched.Word && <ErrorMessageClient>{errors.Word}</ErrorMessageClient>}
                                            <ActionButton onClick={() => {
                                                if(errors.Word && touched.Word) {
                                                    return;
                                                }
                                                submit(values.Word)
                                            }}>
                                            ...
                                            <Inputs>
                                                <div style={{maxWidth: '260px'}}>
                                                    ...
                                                    <Input
                                                        ...
                                                        placeholder={'ETH'}
                                                    />
                                                    {errors.Prize && touched.Prize && <ErrorMessageClient>{errors.Prize}</ErrorMessageClient>}
                                                </div>
                                                <div style={{maxWidth: '260px'}}>
                                                    <p>Coast of try</p>
                                                    <Input
                                                        ...
                                                        placeholder={'ETH'}
                                                    />
                                                    {errors.Attempt_price && touched.Attempt_price && <ErrorMessageClient>{errors.Attempt_price}</ErrorMessageClient>}
                                                </div>
                                            </Inputs>
